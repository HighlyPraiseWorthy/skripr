...
(1) NOVELTY-BASED DE-REPETITION. For each point/theme/figure, ask what a paragraph CONTRIBUTES that has not already been communicated — not whether it looks similar. First mention: keep. Second: keep only if it adds new evidence or a genuinely new angle. Third and beyond: delete unless it carries a NEW fact. A theme reworded across section after section (a co-conspirator's role, one number, one mechanism) with no new information is padding — keep the 1-2 most developed instances and DELETE the rest. Do not reword the survivors.
...
(3) TRIM A WEAK ENDING. If the script ends on a caveat, a methodology note, a hedge, or a "we don't know / the record is silent" line, DELETE that trailing weak material so the script stops on the strong sourced beat that precedes it (a consequence, a figure, a resolved outcome, or the callback). Do not write a new ending; just cut back to the strong line already there.

HARD RULES: Do NOT add any new fact, sentence, or transition. Do NOT invent. Do NOT change the voice, the hook, or the section structure. Do NOT rewrite prose that is fine. You ONLY delete redundancy, delete/soften unsupported inference, and trim a weak tail — the output is always SHORTER than the input. Preserve the opening line exactly. Output ONLY the revised script text, nothing else.`,
      messages: [{
        role: "user",
