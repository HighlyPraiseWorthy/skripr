...
2. RETENTION BEATS: Use three precision mechanics — not generic pattern interrupts:
   ...
   c) CALLBACK THREADING (mandatory — this is graded and it fails often): Plant a specific, concrete detail or thread in the first 20% of the script (ideally the exact thing your hook teased), then RETURN to it in the final 20% and pay it off. The ending must not simply stop — it must land the callback on a REAL SOURCED FACT: the documented outcome, the forfeiture figure, the guilty plea, the actual number. The payoff must be something in the material, never a teased phantom or an implied revelation the facts don't contain. "I can't believe that came back" comes from a real thread resolving on a real fact.
